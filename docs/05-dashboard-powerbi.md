...
- **Dispersão** de `municipio_ano[cobertura_aps]` (eixo X) contra a taxa padronizada de `icvd_municipio[taxa_internacao_padronizada_2023_24]` (eixo Y), um ponto por município. **Filtre `municipio_ano[ano]` em 2023 e 2024 neste visual.** A cobertura vem de duas séries com metodologias diferentes — até 2020 é truncada em 100%, de 2021 em diante não tem teto — e misturá-las no mesmo eixo produziria duas nuvens deslocadas que não se comparam. A cobertura é variável de contexto e **não** compõe o ICVD (ver Seção 3.4 de `03-modelagem.md`).
