...
1. **Corte de eventos mínimos, em dois critérios.**
   - **Principal: 20 internações no total de 2019-2024.** É o corte calibrado na tabela abaixo. O total é o dos **seis anos somados**, não o de cada período isoladamente — 2019 é um ano e 2023-24 são dois, e exigir 20 dentro de cada janela compararia períodos de tamanhos diferentes e derrubaria o ranking para uma fração dos municípios, tornando falsos os números de cobertura populacional da tabela.
   - **Secundário: pelo menos 5 internações em cada um dos dois períodos.** `letalidade` e `prop_amputacao` são razões calculadas dentro de cada período; sem esse piso, um município com 19 internações em 2019 e 1 em 2023-24 passaria no critério principal com um denominador degenerado no período atual, e o ICVD de 2023-24 dele seria decidido por um único caso. Município ausente de um dos períodos também reprova — o indicador de recuperação não existe para quem só tem um lado.
...
O corte de 20 no total dos seis anos remove 22,1% dos municípios do ranking, mas apenas 2,0% da população — porque os municípios abaixo do corte são, por definição, os menores do país. (Se o mesmo 20 fosse exigido **dentro de cada período**, a população mínima subiria para cerca de 28.600 habitantes e o ranking perderia perto de 79% dos municípios e da população — a razão pela qual o critério principal é o total acumulado, e o piso por período é 5 e não 20.) É um compromisso deliberado: cortes menores (10) deixam mais ruído estatístico no ranking; cortes maiores (30, 50) descartam uma fração crescente e desnecessária dos municípios sem ganho proporcional de estabilidade.
